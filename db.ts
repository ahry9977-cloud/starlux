...
async function applyPostgresMigrations(pg: PgPool) {
  ...
  // Reviews: counters + seller response fields
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS helpful_count INTEGER NOT NULL DEFAULT 0`);
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS not_helpful_count INTEGER NOT NULL DEFAULT 0`);
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS report_count INTEGER NOT NULL DEFAULT 0`);
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS seller_response TEXT NULL`);
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS seller_response_at TIMESTAMPTZ NULL`);
  await pg.query(`ALTER TABLE IF EXISTS product_reviews ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NULL DEFAULT CURRENT_TIMESTAMP`);

  // Review interactions (prevent duplicate interactions)
  await pg.query(`
  ...
  `);
  await pg.query(
    `CREATE UNIQUE INDEX IF NOT EXISTS review_interactions_unique ON review_interactions(review_id, user_id, interaction_type)`
  );
  await pg.query(`CREATE INDEX IF NOT EXISTS review_interactions_review_id_idx ON review_interactions(review_id)`);

  // ==============================
  // AI Engine Tables (Phase 1)
  // ==============================
  await pg.query(`
  ...
  `);
  await pg.query(`CREATE INDEX IF NOT EXISTS interaction_logs_user_time_idx ON interaction_logs(user_id, created_at DESC)`);
  await pg.query(`CREATE INDEX IF NOT EXISTS interaction_logs_event_time_idx ON interaction_logs(event_type, created_at DESC)`);
  ...
  `);
  await pg.query(`CREATE INDEX IF NOT EXISTS recommendation_data_user_time_idx ON recommendation_data(user_id, created_at DESC)`);
  await pg.query(`CREATE INDEX IF NOT EXISTS recommendation_data_user_score_idx ON recommendation_data(user_id, score DESC)`);
  await pg.query(`CREATE INDEX IF NOT EXISTS recommendation_data_product_idx ON recommendation_data(product_id)`);

  await pg.query(`
    CREATE TABLE IF NOT EXISTS product_vectors (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      product_id INTEGER NOT NULL,
      model VARCHAR(80) NOT NULL DEFAULT 'tfidf-v1',
      vector TEXT NOT NULL,
      updated_at TIMESTAMPTZ NULL DEFAULT CURRENT_TIMESTAMP
    )
  `);
  await pg.query(`CREATE UNIQUE INDEX IF NOT EXISTS product_vectors_unique ON product_vectors(product_id, model)`);
  ...
  `);
  await pg.query(`CREATE INDEX IF NOT EXISTS ai_model_versions_name_time_idx ON ai_model_versions(name, created_at DESC)`);
  ...
  `);
  await pg.query(`CREATE INDEX IF NOT EXISTS ai_rec_events_time_idx ON ai_recommendation_events(created_at DESC)`);
  await pg.query(`CREATE INDEX IF NOT EXISTS ai_rec_events_user_time_idx ON ai_recommendation_events(user_id, created_at DESC)`);
  await pg.query(`CREATE INDEX IF NOT EXISTS ai_rec_events_product_time_idx ON ai_recommendation_events(product_id, created_at DESC)`);
...
export async function appendChatMessage(_conversationId: number, _role: "system" | "user" | "assistant", _content: string) {
  ...
  if (!content.trim()) return 0 as any;
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const inserted = await db
    .insert(chatMessages)
...
export async function addChatFeedback(_data: {
  ...
  const inserted = await db
    ...
    .values({
      ...
      messageId: Number(_data.messageId),
      userId: _data.userId != null ? Number(_data.userId) : null,
      rating: Number(_data.rating),
      correctedAnswer: _data.correctedAnswer ? String(_data.correctedAnswer) : null,
      notes: _data.notes ? String(_data.notes) : null,
    } as any)
    .returning({ id: chatFeedback.id });
  return Number((inserted as any)?.[0]?.id ?? 0) as any;
}

export async function getChatMemorySnippets(_opts: { userId?: number | null; query: string; limit?: number }) {
  if (_sqlite) return [] as any[];
  const db = await getDb();
  if (!db) return [] as any[];

  const q = String(_opts.query ?? "").trim();
  if (!q) return [] as any[];
  const limit = Math.max(1, Math.min(10, Number(_opts.limit ?? 6)));
  const userId = _opts.userId != null ? Number(_opts.userId) : null;
  ...
  const rows = (await db.execute(sql`
    ...
    WHERE (${userId} IS NULL OR cc.user_id = ${userId})
      AND cm.role IN ('user','assistant')
      AND (cm.content ILIKE ${likeQ} OR COALESCE(cf.corrected_answer,'') ILIKE ${likeQ})
    ORDER BY
      (CASE WHEN cf.corrected_answer IS NOT NULL AND cf.corrected_answer <> '' THEN 0 ELSE 1 END),
      cm.created_at DESC
    LIMIT ${limit}
  `)) as any;

  const list = Array.isArray((rows as any)?.rows) ? (rows as any).rows : Array.isArray(rows) ? rows : [];
  return list;
}
...
export type InteractionEntityType = "product" | "store" | "category" | "search";

export async function logInteraction(_data: {
  userId?: number | null;
  sessionId?: string | null;
  eventType: InteractionEventType;
  entityType?: InteractionEntityType | null;
  ...
  const db = await getDb();
  if (!db) return;
  const userId = _data.userId != null ? Number(_data.userId) : null;
  const sessionId = _data.sessionId != null ? String(_data.sessionId) : null;
  const entityType = _data.entityType != null ? String(_data.entityType) : null;
  const entityId = _data.entityId != null ? Number(_data.entityId) : null;
  const metadata = _data.metadata != null ? JSON.stringify(_data.metadata) : null;

  await db.insert(interactionLogs).values({
    userId,
    sessionId,
    eventType: String(_data.eventType),
    entityType,
    entityId,
    metadata,
  } as any);
...
export async function getAiUserProfile(_userId: number): Promise<{ userId: number; profile: any } | null> {
  if (_sqlite) return null;
  const db = await getDb();
  if (!db) return null;
  const userId = Number(_userId);
  const rows = (await db.select().from(aiUserProfiles).where(eq(aiUserProfiles.userId, userId)).limit(1)) as any[];
  const row = rows?.[0];
  if (!row) return null;
  let parsed: any = {};
  try {
    parsed = row.profileJson ? JSON.parse(String(row.profileJson)) : {};
  } catch {
    parsed = {};
  }
...
export async function upsertAiUserProfile(_userId: number, _patch: any) {
  if (_sqlite) return;
  const db = await getDb();
  if (!db) return;
  const userId = Number(_userId);

  const current = await getAiUserProfile(userId);
  const next = {
    ...(current?.profile ?? {}),
    ...(_patch ?? {}),
  ...
  await db.execute(sql`
    INSERT INTO ai_user_profiles (user_id, profile_json, updated_at)
...
function extractSmartSearchSignals(qRaw: string): {
  keywords: string;
  minPrice?: number;
  maxPrice?: number;
  gamingBoost?: boolean;
  ...
  const priceMinMatch = lower.match(/(?:min|from|على الأقل|ابتداءً من)\s*(\d+)/i);
  const priceMaxMatch = lower.match(/(?:max|to|حتى|إلى)\s*(\d+)/i);

  const gaming = /(gaming|games|للألعاب|العاب|جيمينج)/i.test(lower);
  const battery = /(battery|بطارية|شحن|power)/i.test(lower);
  const camera = /(camera|كاميرا|تصوير|photo)/i.test(lower);

  return {
    keywords: q,
    minPrice: priceMinMatch ? Number(priceMinMatch[1]) : undefined,
    maxPrice: priceMaxMatch ? Number(priceMaxMatch[1]) : undefined,
    gamingBoost: gaming,
...
export async function smartSearchProducts(_query: string, _limit = 20) {
  const q = String(_query ?? "").trim();
  if (!q) return [] as any[];

  // Use existing advanced search if available; apply lightweight intent boosts.
  const signals = extractSmartSearchSignals(q);
  const base = await advancedProductSearch({
    query: signals.keywords,
    minPrice: signals.minPrice,
    maxPrice: signals.maxPrice,
  } as any);

  const items = Array.isArray((base as any)?.items) ? (base as any).items : Array.isArray(base as any) ? (base as any) : [];

  // Re-rank locally using heuristics; this still uses real DB results.
  const scored = items
    .map((p: any) => {
      const title = String(p.title ?? "").toLowerCase();
      const desc = String(p.description ?? "").toLowerCase();
      let bonus = 0;
      if (signals.gamingBoost && /(snapdragon|ram|refresh|hz|gpu|gaming|جيمينج)/i.test(title + " " + desc)) bonus += 2;
      if (signals.batteryBoost && /(mah|battery|بطارية|شحن)/i.test(title + " " + desc)) bonus += 1.5;
      if (signals.cameraBoost && /(camera|mp|كاميرا|تصوير)/i.test(title + " " + desc)) bonus += 1.5;
      return { p, score: bonus };
    })
    .sort((a: any, b: any) => b.score - a.score)
    .slice(0, Math.max(1, Number(_limit)));

  return scored.map((x: any) => x.p);
}

export async function generateRecommendationsForUser(_userId: number, _limit = 12) {
  if (_sqlite) return [] as any[];
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const userId = Number(_userId);
  const limit = Math.max(1, Math.min(50, Number(_limit) || 12));

  // Collect a seed set from: recent views + recent purchases.
  const seed = (await db.execute(sql`
    WITH recent_views AS (
      SELECT pv.product_id AS product_id, MAX(pv.created_at) AS last_at
      ...
      SELECT oi.productid AS product_id, MAX(o.createdat) AS last_at
      FROM orderitems oi
    ...
    SELECT product_id, last_at FROM recent_views
    UNION
    SELECT product_id, last_at FROM recent_purchases
    ORDER BY last_at DESC
    LIMIT 30
  `)) as any;

  const seeds: number[] = Array.isArray((seed as any)?.rows)
    ? (seed as any).rows.map((r: any) => Number(r.product_id)).filter(Boolean)
    : Array.isArray(seed)
      ? seed.map((r: any) => Number((r as any).product_id)).filter(Boolean)
      : [];

  // If no personal history, fallback to trending by product_views.
  const rows = (await db.execute(sql`
    WITH seed_products AS (
      SELECT p.id, p.categoryid, p.storeid
      FROM products p
      WHERE p.id = ANY(${seeds.length ? seeds : [0]})
    ),
    trending AS (
      SELECT pv.product_id AS product_id, COUNT(*)::int AS views
      FROM product_views pv
      GROUP BY pv.product_id
      ORDER BY views DESC
      LIMIT 200
    ...
    SELECT p.*, 
      COALESCE(t.views, 0) AS trend_views,
      CASE
        WHEN EXISTS (SELECT 1 FROM seed_products sp WHERE sp.categoryid = p.categoryid) THEN 3
        WHEN EXISTS (SELECT 1 FROM seed_products sp WHERE sp.storeid = p.storeid) THEN 2
        ELSE 1
      END AS affinity
    FROM products p
    LEFT JOIN trending t ON t.product_id = p.id
    WHERE COALESCE(p.isactive, TRUE) = TRUE
      AND p.id <> ALL(${seeds.length ? seeds : [0]})
    ORDER BY (affinity * 10 + trend_views) DESC, p.id DESC
    LIMIT ${limit}
  `)) as any;

  const resultRows = Array.isArray((rows as any)?.rows) ? (rows as any).rows : Array.isArray(rows) ? rows : [];

  // Persist recommendation snapshot for audits/metrics.
  await db.execute(sql`DELETE FROM recommendation_data WHERE user_id = ${userId}`);
  for (let i = 0; i < resultRows.length; i++) {
    const p = resultRows[i];
    const productId = Number(p.id);
    const score = Number(p.affinity ?? 1) * 10 + Number(p.trend_views ?? 0);
    const reason = Number(p.affinity) >= 3 ? "similar_category" : Number(p.affinity) >= 2 ? "same_store" : "trending";
    await db.insert(recommendationData).values({
      userId,
      productId,
      score,
      reason,
  ...
  // Update AI profile lightweight aggregates.
  await upsertAiUserProfile(userId, {
...
export async function getRecommendationsForUser(_userId: number, _limit = 12) {
  if (_sqlite) return [] as any[];
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const userId = Number(_userId);
  ...
  const rows = (await db
    ...
    .where(eq(recommendationData.userId, userId))
    .orderBy(desc(recommendationData.score))
    .limit(limit)) as any[];
  if (rows.length > 0) return rows;
  await generateRecommendationsForUser(userId, limit);
  return (await db
    .select()
    .from(recommendationData)
    .where(eq(recommendationData.userId, userId))
...
export type AiRecommendationEventType = "impression" | "click";

export async function logRecommendationEvent(_data: {
  userId?: number | null;
  productId: number;
  ...
  await db.insert(aiRecommendationEvents).values({
    ...
    eventType: String(_data.eventType),
    modelVersion: String(_data.modelVersion ?? "heuristic-v1"),
    context: _data.context != null ? JSON.stringify(_data.context) : null,
  } as any);
}

export async function getRecommendationMetrics(opts: { days?: number; modelVersion?: string }) {
  if (_sqlite) {
    return {
      impressions: 0,
      clicks: 0,
      ...
      modelVersion: opts.modelVersion ?? "all",
    };
  }
  ...
  const mv = String(opts.modelVersion ?? "").trim();
  ...
  const res = (await db.execute(sql`
    ...
    WHERE created_at >= NOW() - (${days}::int || ' days')::interval
      AND (${mv} = '' OR model_version = ${mv})
  `)) as any;

  const row = Array.isArray((res as any)?.rows) ? (res as any).rows[0] : Array.isArray(res) ? res[0] : null;
  ...
  const [rows, countRows] = await Promise.all([
    db
      .select({
        ...
        sellerResponse: productReviews.sellerResponse,
        sellerResponseAt: productReviews.sellerResponseAt,
      })
      .from(productReviews)
...
export async function updateRatingSummaryForEntity(_entityType: string, _entityId: number) {
  // Ratings are computed on read via SQL (getRatingSummary). Keep function for router compatibility.
  return;
}
...
export async function createReviewInteraction(_data: any) {
  ...
  const reviewId = Number(_data?.reviewId);
  const userId = Number(_data?.userId);
  const interactionType = String(_data?.interactionType ?? "").trim();
  if (!Number.isFinite(reviewId) || reviewId <= 0) return 0 as any;
  if (!Number.isFinite(userId) || userId <= 0) return 0 as any;
  if (!interactionType) return 0 as any;

  const reportReason = _data?.reportReason != null ? String(_data.reportReason) : null;
  const reportDetails = _data?.reportDetails != null ? String(_data.reportDetails) : null;

  const inserted = await db
    .insert(reviewInteractions as any)
    .values({
      reviewId,
      userId,
      interactionType,
    ...
    .onConflictDoNothing()
    .returning({ id: (reviewInteractions as any).id });
...
export async function getExistingInteraction(_reviewId: number, _userId: number, _interactionType: string) {
  ...
  const reviewId = Number(_reviewId);
  const userId = Number(_userId);
  const interactionType = String(_interactionType ?? "").trim();
  if (!Number.isFinite(reviewId) || reviewId <= 0) return null as any;
  if (!Number.isFinite(userId) || userId <= 0) return null as any;
  if (!interactionType) return null as any;
  ...
  const rows = (await db
    ...
    .where(
      and(
        ...
        eq(reviewInteractions.userId, userId),
        eq(reviewInteractions.interactionType, interactionType)
      )
    )
    .limit(1)) as any[];
...
export async function updateReviewHelpfulCount(_reviewId: number, _increment = 1) {
  ...
  await db
    ...
    .set({
      helpfulCount: sql`${productReviews.helpfulCount} + ${inc}`,
      updatedAt: new Date(),
    } as any)
...
export async function updateReviewNotHelpfulCount(_reviewId: number, _increment = 1) {
  ...
  await db
    .update(productReviews)
    .set({
      notHelpfulCount: sql`${productReviews.notHelpfulCount} + ${inc}`,
      updatedAt: new Date(),
    } as any)
    .where(eq(productReviews.id, reviewId));
}
...
export async function updateReviewReportCount(_reviewId: number, _increment = 1) {
  ...
  await db
    .update(productReviews)
    .set({
      reportCount: sql`${productReviews.reportCount} + ${inc}`,
      updatedAt: new Date(),
    } as any)
    .where(eq(productReviews.id, reviewId));
}
...
export async function addSellerResponse(_reviewId: number, _response: string) {
  ...
  if (!db) throw new Error("Database not available");
  const reviewId = Number(_reviewId);
  const response = String(_response ?? "").trim();
  if (!Number.isFinite(reviewId) || reviewId <= 0) return;
  if (!response) return;
